...
export default class SavePSQL implements SaveInterface {
  ...
  async get(opts: GetOpts): Promise<Update[]> {
    ...
    const query = await client.query(
      `SELECT guild_id, channel_id, message_id, type, ip, name, state, dots, title, offline_title, description, offline_description, color, offline_color, image, offline_image, columns, max_edits, connect_update, disconnect_update, position FROM statuses WHERE ${key}=$1 ORDER BY position, id`,
      [value]
    );
    client.release();
    ...
    try {
      ...
      const r = await client.query(
        "UPDATE statuses SET\
          ...
          color=$13, offline_color=$14, image=$15, offline_image=$16, columns=$17, max_edits=$18, connect_update=$19, disconnect_update=$20, position=$21\
  ...
  async getAll(): Promise<{ [id: string]: Update[] }> {
    const client = await this.pool.connect();
    const query = await client.query(
      "SELECT guild_id, channel_id, message_id, type, ip, name, state, dots, title, offline_title, description, offline_description, color, offline_color, image, offline_image, columns, max_edits, connect_update, disconnect_update, position FROM statuses ORDER BY position, id"
    );
    client.release();
