...
2. SELF-HOST ONLY - Copy `.env.example` to `.env`, fill in `DISCORD_API_KEY` and `DATABASE_PASS`.
3. SELF-HOST ONLY - Register slash commands with Discord:

   ```bash
   cd discord-gamestatus
   docker-compose run --rm bot --register-commands
   ```

4. Run `!status <game> <ip>` or `/status game:<game> ip:<ip>` in Discord to create a status message.
...
3. Request the API key — DM `f.mobile` on Discord, or join the [GameStatus Support Discord](https://discord.gg/VMySf6wtaS).
...
- **Compact status style**: `showPlayers=false` hides the player list for high-density server-list channels.
